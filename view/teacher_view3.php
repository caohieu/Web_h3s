<?php
$queryDanhSachs1 = mysqli_query($conn,"SELECT organization_request_id,status,create_date,student_id  FROM intern_organization_request_assignment WHERE status='1'");
if (mysqli_num_rows($queryDanhSachs1) != 0) {
    ...
    for ($i=0;$i<mysqli_num_rows($queryDanhSachs1);$i++){
        $idRequests1[$i]=$rowDanhSachs1[$i][0];
...
for ($j=0;$j<mysqli_num_rows($queryDanhSachs1);$j++){
    ...
    if (mysqli_num_rows($queryCompanys1) != 0) {
        ...
        $nameCompanys1[$j]=$rowcompany['organization_name'];
    }
